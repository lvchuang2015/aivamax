...
- `Owner Review Package`: generate an internal owner-review ZIP that bundles the evidence snapshot, review records, and final release evidence without marking it as approved distribution.
- `Owner Decision Runbook`: generate the internal owner decision runbook with approval/rejection dry-runs, evidence links, required confirmation phrases, and the guarded post-approval workflow entry without approving or distributing.
- `Distribution Package`: generate the formal approved distribution wrapper only after an approved signoff record.
- `Post Approval Workflow`: owner-only workflow that is blocked before approval, then generates the approved distribution package and records delivery evidence after an approved signoff.
...
- `POST /api/actions/release-owner-review-package`
- `POST /api/actions/release-owner-decision-runbook`
- `POST /api/actions/release-distribution-package`
- `POST /api/actions/release-distribution-delivery-record`
- `POST /api/actions/release-post-approval-workflow`
...
- `GET /api/release-owner-review-package`
- `GET /api/release-owner-decision-runbook`
- `GET /api/release-distribution-package` (read-only distribution readiness status)
- `GET /api/release-post-approval-workflow`

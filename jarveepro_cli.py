...
def run_release_owner_handoff(args: argparse.Namespace) -> int:
    from aivamax_services import release_owner_handoff

    result = release_owner_handoff(
        data_dir=Path(args.data_dir),
        brand_config_path=Path(args.brand_config),
        role=args.role,
    )
    if args.json:
        ...
        print(f"Release owner handoff: {payload.get('handoff_status')}")
        print(f"Release ID: {payload.get('release', {}).get('release_id')} | decision={payload.get('release', {}).get('decision')}")
        if handoff.get("markdown"):
            print(f"Handoff: {handoff['markdown'].get('path')}")
    return 0 if result.get("ok") else 1
...
def run_release_owner_review_package(args: argparse.Namespace) -> int:
    from aivamax_services import release_owner_review_package

    result = release_owner_review_package(
        {
            "decision": args.decision,
            "signer": args.signer,
            "version": args.version,
    ...
    if args.json:
        ...
        payload = result.get("result", {})
        files = payload.get("files", {})
        print(f"Owner review package: {payload.get('package_status')}")
        print(f"Release ID: {payload.get('release', {}).get('release_id')} | decision={payload.get('release', {}).get('decision')}")
        print(f"Evidence files: {payload.get('evidence_file_count')}")
        if files.get("archive"):
            print(f"Archive: {files['archive'].get('path')}")
    return 0 if result.get("ok") else 1
...
def run_release_post_approval_workflow(args: argparse.Namespace) -> int:
    from aivamax_services import release_post_approval_workflow

    result = release_post_approval_workflow(
        {
            "dry_run": args.dry_run,
            "delivery_owner": args.delivery_owner,
            "recipient_label": args.recipient_label,
    ...
    if args.json:
        ...
        payload = result.get("result", {})
        print(f"Post-approval workflow: {payload.get('workflow_status')}")
        print(f"Release ID: {payload.get('release_id')} | decision={payload.get('decision')} | distribution={payload.get('distribution_status')}")
        report = payload.get("report", {})
        if report.get("markdown"):
            print(f"Report: {report['markdown'].get('path')}")
    return 0 if result.get("ok") else 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    release_owner_handoff_parser = sub.add_parser("release-owner-handoff", help="Generate the AIvaMax owner release handoff report without approving or distributing.")
    ...
    release_owner_review_package_parser = sub.add_parser("release-owner-review-package", help="Generate an internal AIvaMax owner review ZIP package without approving or distributing.")
    release_owner_review_package_parser.add_argument("--role", default="owner_admin", choices=["owner_admin", "team_operator", "instructor_private", "student_public"])
    release_owner_review_package_parser.add_argument("--decision", default="approved", choices=["pending_review", "approved", "rejected"])
    release_owner_review_package_parser.add_argument("--signer", default="owner_admin")
    release_owner_review_package_parser.add_argument("--version", default="course-factory-v1")
    release_owner_review_package_parser.add_argument("--notes", default="")
    release_owner_review_package_parser.add_argument("--confirmation", default="")
    ...
    release_post_approval_parser = sub.add_parser("release-post-approval-workflow", help="Run the guarded post-approval distribution and delivery workflow after owner approval.")
    release_post_approval_parser.add_argument("--delivery-owner", default="owner_admin")
    release_post_approval_parser.add_argument("--recipient-label", default="internal_distribution_recipient")
    release_post_approval_parser.add_argument("--delivery-channel", default="manual_handoff")
    release_post_approval_parser.add_argument("--notes", default="")
    release_post_approval_parser.add_argument("--dry-run", action="store_true")
    release_post_approval_parser.add_argument("--role", default="owner_admin", choices=["owner_admin", "team_operator", "instructor_private", "student_public"])
    release_post_approval_parser.add_argument("--json", action="store_true")
    release_post_approval_parser.set_defaults(func=run_release_post_approval_workflow)
